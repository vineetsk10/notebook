...
define([
     ...
     * @method execute
     */
    var old_execute = function (tis, stop_on_error) {
        if (!tis.kernel) {
        ...
        if (stop_on_error === undefined) {
            if (tis.metadata !== undefined && 
                    tis.metadata.tags !== undefined) {
                if (tis.metadata.tags.indexOf('raises-exception') !== -1) {
                    stop_on_error = false;
                } else {
                    stop_on_error = true;
                }
        ...
        tis.clear_output(false, true);
        var old_msg_id = tis.last_msg_id;
        if (old_msg_id) {
            tis.kernel.clear_callbacks_for_msg(old_msg_id);
            delete CodeCell.msg_cells[old_msg_id];
            tis.last_msg_id = null;
        }
        if (tis.get_text().trim().length === 0) {
            // nothing to do
            tis.set_input_prompt(null);
            return;
        }
        tis.set_input_prompt('*');
        tis.element.addClass("running");
        var callbacks = tis.get_callbacks();
        
        tis.last_msg_id = tis.kernel.execute(tis.get_text(), callbacks, {silent: false, store_history: true,
            stop_on_error : stop_on_error});
        CodeCell.msg_cells[tis.last_msg_id] = tis;
        tis.render();
        tis.events.trigger('execute.CodeCell', {cell: tis});
        var that = tis;
        ...
        tis.events.on('finished_iopub.Kernel', handleFinished);
    };

    CodeCell.prototype.execute = function (stop_on_error) {
        if (this.metadata === undefined || this.metadata.run_control === undefined || 
            !this.metadata.run_control.frozen) {
            console.log("Applying old execute");
            old_execute(this, stop_on_error);
            console.log("Adjusting metadata");
            this.metadata.editable = false;
            this.metadata.deletable = false;
            $.extend(true, this.metadata, {run_control: {frozen: true}});
            this.code_mirror.setOption('readOnly', !this.metadata.editable);
            var prompt = this.element.find("div.input_area");
            prompt.css("background-color", "#f0feff");
        }
    }
